...
class _CodingArenaScreenState extends State<CodingArenaScreen>
  ...
  final _groqDio = Dio(BaseOptions(
  ...
  Future<void> _submitSolution() async {
    ...
    try {
      ...
      final response = await _groqDio.post('/chat/completions', data: {
        ...
        'messages': [
          {'role': 'system', 'content': 'You are a Python code evaluator. Respond ONLY with valid JSON.'},
      ...
      debugPrint('[CODING] Groq eval error: $e');
      // Fallback scoring
    ...
    try {
      ...
      if (uid != null) {
        await FirebaseFirestore.instance.collection('users').doc(uid).update({
          ...
          'courseProgress.${widget.subjectId}.${widget.chapterId}.${widget.lessonId}': true,
          'codingStats.totalProblems': FieldValue.increment(_problems.length),
          'codingStats.totalScore': FieldValue.increment(_totalScore),
          'codingStats.linesWritten': FieldValue.increment(_linesWritten),
        });
      }
    } catch (_) {}
