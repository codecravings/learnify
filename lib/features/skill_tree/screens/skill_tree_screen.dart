...
class _SkillTreeScreenState extends State<SkillTreeScreen> {
  ...
  Future<void> _loadTree() async {
    ...
    if (uid == null) {
      // No user — still show course structure as locked nodes
      final nodes = _buildNodes({});
      if (mounted) setState(() { _nodes = nodes; _loading = false; });
      return;
    }

    Map<String, dynamic> studiedTopics = {};

    // Try Firestore first
    try {
      final doc = await FirebaseFirestore.instance.collection('users').doc(uid).get();
      studiedTopics = doc.data()?['studiedTopics'] as Map<String, dynamic>? ?? {};
    } catch (e) {
      debugPrint('[SKILL_TREE] Firestore failed, trying cache: $e');
      ...
      try {
        ...
        if (cached != null) {
          ...
          studiedTopics = data['studiedTopics'] as Map<String, dynamic>? ?? {};
        }
      } catch (_) {}
    }

    // Always build nodes — even with empty studiedTopics we show course structure
